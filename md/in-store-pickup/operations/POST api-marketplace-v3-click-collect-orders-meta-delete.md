...
**Удалить метаданные сборочных заданий{{ /api/marketplace/v3/click-collect/orders/meta/delete }}**

теги: `Метаданные Самовывоз`

**Базовый URL:** `https://marketplace-api.wildberries.ru`

**Полный путь:** `POST https://marketplace-api.wildberries.ru/api/marketplace/v3/click-collect/orders/meta/delete`
...
- `key` **(required)** — string; пример: `imei`. Тип метаданных для удаления (`imei`, `uin`, `gtin`, `sgtin`). Передаётся только одно значение
- `ordersIds` **(required)** — array<integer>. Список ID сборочных заданий
...
- `results` **(required)** — array<$ref: api.OrdersResponse>
