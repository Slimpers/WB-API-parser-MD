...
**Полный путь:** `GET /api/v3/supplies/{supplyId}`
...
- `isB2b` — boolean. Признак B2B-продажи:
- `done` — boolean. Флаг закрытия поставки:
- `createdAt` — string (date-time); пример: `2022-05-04T07:56:29Z`. Дата создания поставки (RFC3339)
- `closedAt` — string (date-time); пример: `2022-05-04T07:56:29Z`. Дата закрытия поставки (RFC3339)
- `scanDt` — string (date-time); пример: `2022-05-04T07:56:29Z`. Дата скана поставки (RFC3339)
- `name` — string; пример: `Тестовая поставка`. Наименование поставки
- `cargoType` — integer; enum: [0, 1, 2, 3]. Тип товара:
- `crossBorderType` — integer (int32); enum: [0, 1]; пример: `1`. Тип поставки:
- `destinationOfficeId` — integer (int64); пример: `123`. ID склада назначения поставки. Если `null`, склад назначения не указан
- `recommendedWhId` — integer (int64); пример: `123569`. ID рекомендуемого склада для приёмки поставки для Москвы и МО. <br>
