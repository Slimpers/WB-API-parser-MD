...
**Полный путь:** `GET /api/v1/warehouse_remains/tasks/{task_id}/download`
...
- array of: object
  - `brand` — string; пример: `Wonderful`. Бренд
  - `subjectName` — string; пример: `Фотоальбомы`. Название предмета
  - `vendorCode` — string; пример: `41058/прозрачный`. Артикул продавца
  - `nmId` — integer; пример: `183804172`. Артикул WB
  - `barcode` — string; пример: `2037031652319`. Баркод
  - `techSize` — string; пример: `0`. Размер
  - `volume` — number; пример: `1.33`. Объём, л
  - `warehouses` — array<object>. Остатки на складах и товары в пути. Будут в ответе только при ненулевом `quantity`
    - *(элементы)*
      - `warehouseName` — string; пример: `Невинномысск`. Название склада
      - `quantity` — integer; пример: `134`. Количество, шт.
