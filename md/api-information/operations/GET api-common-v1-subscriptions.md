...
**Получить информацию о подписке Джем{{ /api/common/v1/subscriptions }}**

`operationId`: getCommonV1Subscriptions  
теги: `Информация о продавце`

**Полный путь:** `GET /api/common/v1/subscriptions`
...
- `state` **(required)** — string; enum: ["active", "inactive"]; пример: `active`. Статус подписки:
- `activationSource` **(required)** — string; enum: ["constructor", "jam"]; пример: `jam`. Источник подключения подписки:
- `level` **(required)** — string; enum: ["standard", "advanced", "premium"]; пример: `premium`. Уровень подписки:
- `since` **(required)** — string (date-time); пример: `2026-03-16T08:38:08.056406Z`. Дата и время первой активации подписки. Не меняется при продлении или повторной активации
- `till` **(required)** — string (date-time); пример: `2026-04-25T14:44:28.393587Z`. Дата и время окончания подписки
