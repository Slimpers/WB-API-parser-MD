...
Если ответ `Успешно` (`200`), но какие-то карточки не обновились, проверьте [список несозданных карточек товаров](./work-with-products#tag/Kartochki-tovarov/paths/~1content~1v2~1cards~1error~1list/post).<br>
...
- array of: object
  ...
  - `sizes` **(required)** — array<object>. Массив размеров<br>
    - *(элементы)*
      - `chrtID` — integer. ID размера для данного артикула WB<br>
      - `techSize` — string. Размер товара (например, XL, S, 45)
      - `wbSize` — string. Российский размер товара
      - `price` — integer. Цена товара, ₽<br>
